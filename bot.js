...
Actions.checkConditions = function (guild, member, user, cmd) {
  ...
  // Konwertowanie numerów permisji na odpowiedni format PermissionsBitField
  const permissions = new PermissionsBitField(cmd.permissions);
  const permissions2 = new PermissionsBitField(cmd.permissions2);

  switch (restriction) {
    case 0:
    case 1: {
      if (isServer) {
        // Łączenie permisji (cmd.permissions i cmd.permissions2) i sprawdzenie ich
        return this.checkPermissions(
          member,
          permissions.bitfield | permissions2.bitfield
        );
      }
