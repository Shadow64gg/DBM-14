...
module.exports = {
  ...
  meta: {
    ...
    downloadURL:
      "https://github.com/Shadow64gg/DBM-14/blob/DBM-14/actions/generate_MOD.js",
  },
...
<style>
  ...
  init() {
    ...
    glob.onChangeType = function (event) {
      ...
      document.getElementById("passwordSection").style.display =
        value === "password" ? null : "none";
      document.getElementById("nicknameSection").style.display =
        value === "nickname" ? null : "none";
      document.getElementById("licenseKeySection").style.display =
        value === "licensekey" ? null : "none";
      document.getElementById("fullnameSection").style.display =
        value === "fullname" ? null : "none";
    };
  ...
  async action(cache) {
    ...
    const licenseKeyLength = parseInt(
      this.evalMessage(data.licenseKeyLength, cache),
      10
    );
    ...
    const generatesFilePath = path.join(
      __dirname,
      "..",
      "data",
      "generates.json"
    ...
    if (!fs.existsSync(generatesFilePath)) {
      console.log(
        "generates.json not found. Creating a new file with default data."
      );
      ...
      fs.writeFileSync(
        generatesFilePath,
        JSON.stringify(generatesData, null, 2),
        "utf8"
      );
    } else {
