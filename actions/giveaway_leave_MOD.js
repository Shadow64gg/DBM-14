...
<style>
  ...
  async action(cache) {
    ...
    if (!member) {
      console.error("[Giveaway Leave] User not found.");
      return this.callNextAction(cache);
    }

    const giveawayID = this.evalMessage(data.giveawayid, cache);
    if (!giveawayID) {
      console.error("[Giveaway Leave] No giveaway ID provided.");
      return this.callNextAction(cache);
    }

    const guildId = member.guild?.id;
    if (!guildId) {
      console.error("[Giveaway Leave] Server ID not found.");
      return this.callNextAction(cache);
    }

    const filePath = path.join(__dirname, "../data/giveaways.json");
    if (!fs.existsSync(filePath)) {
      console.error("[Giveaway Leave] The giveaways.json file does not exist.");
      return this.callNextAction(cache);
    }
    ...
    try {
      ...
      console.error("[Giveaway Leave] Error reading giveaways.json:", err);
      return this.callNextAction(cache);
    }

    if (!giveaways[guildId]) {
      console.error(
        "[Giveaway Leave] There are no active giveaways for this server."
      );
      return this.callNextAction(cache);
    }
    ...
    if (!giveaway) {
      console.error(
        `[Giveaway Leave] No active giveaway with ID: ${giveawayID} found.`
      );
      return this.callNextAction(cache);
    }
    ...
    try {
      ...
      console.error("[Giveaway Leave] Error writing to giveaways.json:", err);
    }
