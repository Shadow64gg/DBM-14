module.exports = {
  ...
  meta: {
    ...
    downloadURL:
      "https://github.com/dbm-network/mods/blob/master/actions/set_time_restriction_MOD.js",
  },
...
<div>
  ...
  async mod(DBM) {
    let Cooldown;
    DBM.Actions.LoadTimeRestriction = async function LoadTimeRestriction(
      cache
    ) {
    ...
    DBM.Actions.TimeRestriction = async function TimeRestriction(
      TRData,
      cmd,
      cache
    ) {
      ...
      switch (restrict) {
        case 0:
          ...
          if (Cooldown[cmd.name][author.id]) {
            const expirationTime =
              Cooldown[cmd.name][author.id] + Cooldown[cmd.name].cooldown;
        ...
        case 1: {
          ...
          if (Cooldown[cmd.name][author.id][channelId]) {
            const expirationTime =
              Cooldown[cmd.name][author.id][channelId] +
