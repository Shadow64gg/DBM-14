  ...
  subtitle(data, presets) {
    return `Create Thread Named "${data.threadName}" from ${
      data.fromTarget._index === 0
        ? presets.getChannelText(
            data.fromTarget?.channel ?? 0,
            data.fromTarget?.channelVarName
          )
        : presets.getMessageText(
            data.fromTarget?.message ?? 0,
            data.fromTarget?.messageVarName
          )
    }`;
  },
...
<store-in-variable allowNone selectId="storage" variableInputId="storageVarName" variableContainerId="varNameContainer2"></store-in-variable>`;
  ...
  async action(cache) {
    ...
    if (data.fromTarget._index === 0) {
      messageOrChannel = await this.getChannelFromData(
        data.fromTarget.channel,
        data.fromTarget.channelVarName,
        cache
      );
    } else {
      messageOrChannel = await this.getMessageFromData(
        data.fromTarget.message,
        data.fromTarget.messageVarName,
        cache
      );
    }
    ...
    if (messageOrChannel !== null) {
      if (Array.isArray(messageOrChannel)) {
        this.callListFunc(messageOrChannel, "startThread", [
          threadOptions,
