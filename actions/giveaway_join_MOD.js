module.exports = {
  ...
  async action(cache) {
    ...
    if (!member) {
      console.error("Giveaway Join: Nie znaleziono użytkownika.");
      return this.callNextAction(cache);
    }

    const giveawayID = this.evalMessage(data.giveawayid, cache);
    if (!giveawayID) {
      console.error("Giveaway Join: Nie podano ID giveaway.");
      return this.callNextAction(cache);
    }

    const guildId = member.guild?.id;
    if (!guildId) {
      console.error("Giveaway Join: Nie znaleziono ID serwera.");
      return this.callNextAction(cache);
    }

    const filePath = path.join(__dirname, "../data/giveaways.json");
    if (!fs.existsSync(filePath)) {
      console.error("Giveaway Join: Plik giveaways.json nie istnieje.");
      return this.callNextAction(cache);
    }
    ...
    try {
      ...
      console.error("Giveaway Join: Błąd odczytu giveaways.json:", err);
      return this.callNextAction(cache);
    }

    if (!giveaways[guildId]) {
      console.error("Giveaway Join: Brak aktywnych giveaway dla tego serwera.");
      return this.callNextAction(cache);
    }
    ...
    if (!giveaway) {
      console.error(
        `Giveaway Join: Nie znaleziono aktywnego giveaway o ID ${giveawayID}.`
      );
      return this.callNextAction(cache);
    }
    ...
    try {
      ...
      console.error("Giveaway Join: Błąd zapisu do giveaways.json:", err);
    }
