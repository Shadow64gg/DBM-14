module.exports = {
  ...
  subtitle(data, presets) {
    return `${presets.getSendTargetText(data.channel, data.varName2)}: ${
      data.varName
    }`;
  },
...
<store-in-variable allowNone selectId="storage3" variableInputId="varName3" variableContainerId="varNameContainer3"></store-in-variable>`;
  ...
  async action(cache) {
    ...
    const target = await this.getSendTargetFromData(
      data.channel,
      data.varName2,
      cache
    );
    ...
    if (Array.isArray(target)) {
      this.callListFunc(target, "send", [{ embeds: [embed] }]).then(() =>
