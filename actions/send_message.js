...
<tab-system style="margin-top: 20px;">
  ...
  <tab label="Settings" icon="cogs">
    <div style="padding: 8px; width: 535px;">
  ...
  async action(cache) {
    ...
    const onComplete = (resultMsg) => {
      // Jeśli resultMsg ma właściwość resource.message, to używamy jej jako ostatecznego wyniku
    ...
    if (data.dontSend) {
      ...
      // Jeśli nie wysyłasz wiadomości, wykonaj deferUpdate na interakcji
      if (
        cache.interaction &&
      ...
      // Jeśli interakcja jeszcze nie została odpowiedziana – używamy update (lub reply, jeśli update nie jest dostępne)
      ...
      // Jeśli interakcja już została odpowiedziana – używamy editReply
      ...
      if (data.ephemeral === true) {
        // Jeśli używasz opcji ephemeral, ustaw ją bezpośrednio
