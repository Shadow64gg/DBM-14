module.exports = {
  ...
  meta: {
    version: "3.2.4",
    preciseCheck: true,
    author: "Shadow / Rozbudowane przez Ciebie",
  ...
  async action(cache) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.error(
          "[Write In Json File] Error reading or parsing JSON file:",
          error
        );
        jsonData = {};
      }
      ...
      case 8:
        {
          if (keyPath && newName) {
            ...
            if (!result) {
              console.error(
                "[Write In Json File] Failed to rename property. Make sure the key exists."
              );
            }
          }
        }
    ...
    try {
      ...
      console.error(
        "[Write In Json File] Error while writing JSON file:",
        error
      );
    }
