module.exports = {
  ...
  meta: {
    ...
    downloadURL:
      "https://github.com/dbm-network/mods/blob/master/actions/canvas_create_primitive_MOD.js",
  },
    ...
    switch (info) {
      case 0:
        return data.color
          ...
          : "No color circle has been created";
      case 1:
        return data.color
          ? `Create Rectangle with Color ${data.color}`
          : "No color rectangle has been created";
      case 2:
        return data.color
          ? `Create Triangle with Color ${data.color}`
          : "No color triangle has been created";
      case 3:
        return data.color
          ? `Create Hexagon with Color ${data.color}`
          : "No color hexagon has been created";
      case 4:
        return data.color
          ? `Create Pentagon with Color ${data.color}`
          : "No color pentagon has been created";
      case 5:
        return data.color
          ? `Create Ellipse with Color ${data.color}`
          : "No color ellipse has been created";
      case 6:
        return data.color
          ? `Create Star with Color ${data.color}`
          : "No color star has been created";
      default:
        return "";
    }
  },
      ...
      case 4:
        ...
        for (let i = 1; i <= 5; i++) {
          ctx.lineTo(
            ...
            centerY + radius * Math.sin(angleStep * i)
          );
        }
        ctx.closePath();
        ctx.fillStyle = color;
