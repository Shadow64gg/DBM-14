...
<div style="float: left; width: 80%;">
  ...
  action(cache) {
    ...
    const presenceOptions = {
      status: afk === 0 ? "idle" : "online", // 'idle' dla AFK, 'online' dla aktywnego
      activities: [
        {
          name: afk === 0 ? "AFK" : "Active", // Opcjonalnie ustaw aktywność bota
          type: "WATCHING", // Typ aktywności, możesz go dostosować
        },
      ],
    };

    // Ustawienie statusu bez .then() i .catch()
    botClient.setPresence(presenceOptions);

    // Przejście do następnej akcji
