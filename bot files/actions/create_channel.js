module.exports = {
  ...
  meta: {
    ...
    authorUrl: null,
    downloadUrl:
      "https://github.com/Gotowka/mydbm/blob/v3/actions/create_channel.js",
  },
...
<store-in-variable allowNone selectId="storage" variableInputId="varName" variableContainerId="varNameContainer"></store-in-variable>`;
  ...
  async action(cache) {
    ...
    const server =
      (await this.getServerFromData(data.server, data.varName2, cache)) ??
      cache.server;
    ...
    if (data.position) {
      channelData.position = parseInt(
        this.evalMessage(data.position, cache),
        10
      );
    }
    if (data.categoryID) {
